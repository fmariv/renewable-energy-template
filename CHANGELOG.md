...
- Friendly `GET /` response (`service: "SPAI API"`, name: `renewable-energy`).
...
- SSR no longer treats `GET /` as an images payload; loads `/aoi` via `safeFetch`.
- Project name typo fixed: `renewable-energy-templatee` → `renewable-energy-template`.
